...
export class NonSerialisedInventoryItemDutchPartDisplayNameRule
  implements IRule<NonSerialisedInventoryItem>
{
  ...
  constructor(m: M) {
    ...
    this.roleType = m.NonSerialisedInventoryItem.DutchPartDisplayName;
  ...
  derive(nonSerialisedInventoryItem: NonSerialisedInventoryItem) {
    ...
    return `${nonSerialisedInventoryItem.FacilityName} ${nonSerialisedInventoryItem.Part?.DutchName ?? nonSerialisedInventoryItem.Part?.DisplayName}, ${supplierInfo}, QOH: ${nonSerialisedInventoryItem.QuantityOnHand}`;
  }
}
