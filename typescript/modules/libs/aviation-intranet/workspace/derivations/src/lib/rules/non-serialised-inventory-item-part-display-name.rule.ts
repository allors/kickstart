...
export class NonSerialisedInventoryItemPartDisplayNameRule
  implements IRule<NonSerialisedInventoryItem>
{
  ...
  constructor(m: M) {
    ...
    this.roleType = m.NonSerialisedInventoryItem.PartDisplayName;
  ...
  derive(nonSerialisedInventoryItem: NonSerialisedInventoryItem) {
    ...
    return `${nonSerialisedInventoryItem.FacilityName} ${nonSerialisedInventoryItem.Part?.DisplayName}, ${supplierInfo}, QOH: ${nonSerialisedInventoryItem.QuantityOnHand}`;
  }
}
