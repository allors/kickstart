...
export class NonUnifiedPartInternalSellingPriceRule
  implements IRule<NonUnifiedPart>
{
  ...
  constructor(private m: M) {
    ...
    this.roleType = m.NonUnifiedPart.SuggestedInternalSellingPrice;
  }
  ...
  derive(nonUnifiedPart: NonUnifiedPart) {
    ...
    const sellingPrice =
      Math.round(
        maxPurchasePrice *
          (1 + parseFloat(settings.InternalPartSurchargePercentage) / 100) *
          100
      ) / 100;
