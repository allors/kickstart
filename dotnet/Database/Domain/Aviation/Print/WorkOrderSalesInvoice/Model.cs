...
namespace Allors.Database.Domain.Print.WorkOrderSalesInvoiceModel
{
    using System.Linq;
    using SalesInvoiceModel;
    using WorkTaskModel;
    ...
    public class Model
    {
        public Model(SalesInvoice invoice)
        {
            ...
            this.WorkTasks = invoice.WorkEfforts.Select(v => new Print.WorkTaskModel.Model((WorkTask)v)).ToArray();

            var paymentTerm = new InvoiceTermTypes(invoice.Strategy.Transaction).PaymentNetDays;
        ...
        public Print.WorkTaskModel.Model [] WorkTasks { get; }
    }
}
