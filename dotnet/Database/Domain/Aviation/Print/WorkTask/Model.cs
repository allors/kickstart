...
namespace Allors.Database.Domain.Print.WorkTaskModel
{
    ...
    public class Model
    {
        public Model(WorkTask workTask)
        {
            ...
            this.WorkEffortInvoiceItemAssignments = workTask.WorkEffortInvoiceItemAssignmentsWhereAssignment.Where(v => v.WorkEffortInvoiceItem.InvoiceItemType.IsOther).Select(v => new SalesInvoiceItemAssignmentModel(v)).ToArray();

            if (workTask.ExistMaintenanceAgreement)
            {
                if (workTask.BillableAmountOfTimeInHours.Value > 0)
                {
                    this.ExtraLabour = new ExtraLabourModel(workTask);
                }
            }
            else
            {
        ...
        public SalesInvoiceItemAssignmentModel[] WorkEffortInvoiceItemAssignments { get; }

        public TimeEntryByBillingRateModel[] TimeEntriesByBillingRate { get; }

        public ExtraLabourModel ExtraLabour { get; }
    }
}
